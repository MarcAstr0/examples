...
import { LIKE_QUESTION } from '../../common/graphql-queries';
...
const QuestionItem = forwardRef((props: QuestionItemProps, ref: ForwardedRef<HTMLDivElement>) => {
  ...
  const [Like] = useMutation(LIKE_QUESTION);
        ...
        <Box display='flex' alignItems='center'>
          ...
          <Box paddingRight={2} display='flex' flexDirection='column' justifyContent='center' alignItems='center'>
            <IconButton aria-label="delete" onClick={onCountChange}>
              {question.likes === 0 ? <FavoriteBorder className={classes.heartColor} /> : <Favorite className={classes.heartColor} />}
            </IconButton>
            {question.likes > 0 && <Typography style={{ color: '#f44336' }} className={classes.postedAt}>
              {question.likes}
            </Typography>}
          </Box>
        </Box>
